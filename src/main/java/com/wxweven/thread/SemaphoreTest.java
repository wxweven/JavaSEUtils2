...
/**
 * Java 5.0里新加了4个协调线程间进程的同步装置，它们分别是： Semaphore, CountDownLatch,
 * CyclicBarrier和Exchanger. 本例主要介绍Semaphore。 Semaphore是用来管理一个资源池的工具，可以看成是个通行证，
 * 线程要想从资源池拿到资源必须先拿到通行证， 如果线程暂时拿不到通行证，线程就会被阻断进入等待状态。
 */
public class SemaphoreTest {
	/**
	 * 模拟资源池的类 只为池发放2个通行证，即同时只允许2个线程获得池中的资源。
	 */
